...
trait Urls extends algebra.Urls { this: EndpointsWithCustomErrors =>
  ...
  implicit lazy val applicativeRequestExtractor: Applicative[RequestExtractor] =
    new Applicative[RequestExtractor] {
      def apply[A, B](
          mf: RequestExtractor[A => B],
          ma: RequestExtractor[A]
      ): RequestExtractor[B] =
  ...
  implicit final class ApplicativeMapSyntax[F[_], A](fa: F[A]) {
    @inline def map[B](f: A => B)(implicit applicative: Applicative[F]): F[B] =
  ...
  implicit lazy val queryStringPartialInvariantFunctor
    ...
    new PartialInvariantFunctor[QueryString] {
      def xmapPartial[A, B](
        ...
        new QueryString[B] {
          def decode(qs: Map[String, Seq[String]]): Validated[B] =
            fa.decode(qs).flatMap(f)
    ...
    new PartialInvariantFunctor[QueryStringParam] {
      def xmapPartial[A, B](
        ...
        new QueryStringParam[B] {
          def decode(name: String, qs: Map[String, Seq[String]]): Validated[B] =
            fa.decode(name, qs).flatMap(f)
          def encode(name: String, b: B): Map[String, Seq[String]] =
            fa.encode(name, g(b))
        }
      override def xmap[A, B](
          fa: QueryStringParam[A],
  ...
  def staticPathSegment(segment: String): Path[Unit] =
    new Path[Unit] {
      def decode(
          segments: List[String]
      ): Option[(Validated[Unit], List[String])] =
        segments match {
