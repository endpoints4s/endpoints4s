...
non-existing `UUID`). The `option` method takes a `Response[A]` description and
...
be extended according to application-specific needs. We also saw how support for custom data types (e.g. `UUID`) can
be introduced.
