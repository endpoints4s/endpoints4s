...
Here is the semantics given by the `endpoints.play.routing.Endpoints` trait:
...
The aim of the `endpoints.play.routing.Endpoints` trait is to provide a Play router for a given set of HTTP endpoints. So,
...
and `endpoints.play.routing.Endpoints` are *object algebras*. Note that we use an encoding borrowed from C. Hofer [2], which encodes
