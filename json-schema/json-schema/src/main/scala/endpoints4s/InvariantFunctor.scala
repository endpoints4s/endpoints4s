...
trait InvariantFunctor[F[_]] {
  ...
  /** Transforms an `F[A]` value into an `F[B]` value given a pair of functions from `A` to `B` and
    * from `B` to `A`.
    *
    * @see
    *   [[http://julienrf.github.io/endpoints/algebras/endpoints.html#transforming-and-refining-url-constituents Some examples]]
    */
...
trait InvariantFunctorSyntax {
  ...
  implicit class InvariantFunctorSyntax[A, F[_]](val fa: F[A])(implicit
    ...
    /** Transforms an `F[A]` value into an `F[B]` value given a pair of functions from `A` to `B`
      * and from `B` to `A`.
      *
      * @see
      *   [[http://julienrf.github.io/endpoints/algebras/endpoints.html#transforming-and-refining-url-constituents Some examples]]
...
/** Given a type constructor `F`, a partial function `A => Validated[B]` and a total function `B =>
  * A`, turns an `F[A]` into an `F[B]`.
  *
  * A partial invariant functor is an invariant functor whose covariant transformation function is
  * total (ie, `A => Valid[B]`).
  */
trait PartialInvariantFunctor[F[_]] extends InvariantFunctor[F] {

  /** Transforms an `F[A]` value into an `F[B]` value given a partial function from `A` to `B`, and
    * a total function from `B` to `A`.
    *
...
trait PartialInvariantFunctorSyntax extends InvariantFunctorSyntax {
  implicit class PartialInvariantFunctorSyntax[A, F[_]](val fa: F[A])(implicit
    ...
    /** Transforms an `F[A]` value into an `F[B]` value given a partial function from `A` to `B`,
      * and a total function from `B` to `A`.
      *
      * This is useful to ''refine'' the type `A` into a possibly smaller type `B`.
      *
      * @see
