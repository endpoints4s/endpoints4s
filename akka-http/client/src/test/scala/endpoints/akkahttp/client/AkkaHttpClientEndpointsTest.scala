    ...
    with algebra.client.ChunkedJsonEntitiesTestSuite[TestClient]
{
  ...
  def callStreamedEndpoint[A, B](endpoint: streamingClient.Endpoint[A, streamingClient.Chunks[B]], req: A): Future[Seq[Either[String, B]]] =
    Source.futureSource(endpoint(req)).map(Right(_)).recover { case NonFatal(t) => Left(t.toString) }.runWith(Sink.seq)
