...
trait ChunkedEntities
  ...
  def bytesChunksResponse: ResponseEntity[Chunks[Array[Byte]]] =
    EntityEncoder.encodeBy(`Content-Type`(MediaType.application.`octet-stream`))(stream =>
...
trait ChunkedJsonEntities
  ...
  def jsonChunksRequest[A](
    ...
    req => Effect.pure(Right(req.bodyAsText.evalMap(s => decoder.decode(s).fold(Effect.pure, es => Effect.raiseError[A](new Throwable(es.mkString(", ")))))))
  }
