...
trait ChunkedEntities
  ...
  def textChunksRequest: RequestEntity[Chunks[String]] =
    req => Effect.pure(Right(req.bodyAsText))

  def textChunksResponse: ResponseEntity[Chunks[String]] =
    EntityEncoder.encodeBy()(stream =>
      Entity(stream.through(fs2.text.utf8Encode))
    )
  ...
  def bytesChunksResponse: ResponseEntity[Chunks[Array[Byte]]] =
    EntityEncoder.encodeBy(
      ...
      Entity(stream.flatMap(arr => fs2.Stream.chunk(Chunk.array(arr))))
    )
...
trait ChunkedJsonEntities
  ...
  def jsonChunksResponse[A](
    ...
    EntityEncoder.encodeBy(`Content-Type`(MediaType.application.`json`))(
      stream => Entity(stream.map(encoder.encode).through(fs2.text.utf8Encode))
    )
  }
